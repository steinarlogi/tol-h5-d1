...
window.onload = function init()
{
    ...
    window.addEventListener("keydown", function(e){
        switch( e.keyCode ) {
            ...
            case 57: // 9:
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "9: TEST";
                break;
            case 48: // 0:
                view = 0;
                document.getElementById("Viewpoint").innerHTML = "0: TEST";
                break;
            ...
            case 38:    // up arrow
                if (view != 0 && view != 9) {
                    ...
                    loc = add(loc, vec2(Math.cos(rotation), Math.sin(rotation))); 
                }
                break;
            case 40:    // down arrow
                if (view != 0 || view != 9) {
...
function render()
{
	...
	case 8:
	    ...
	    break;
    case 9:
        mv = lookAt(vec3(-40.0, 140.0, 16.0 + height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0));
        drawScenery( mv );
	    mv = mult( mv, translate( carXPos, carYPos, 0.0 ) );
	    mv = mult( mv, rotateZ( -carDirection ) ) ;
	    drawCar( mv );
    ...
    case 0:
        console.log(loc);
        mv = lookAt(vec3(loc[0], loc[1], 5.0), add(vec3(loc[0], loc[1], 5.0), vec3(Math.cos(rotation), Math.sin(rotation), 0.0)), vec3(0.0, 0.0, 1.0));
        drawScenery( mv );
	    mv = mult( mv, translate( carXPos, carYPos, 0.0 ) );
	    mv = mult( mv, rotateZ( -carDirection ) ) ;
        drawCar(mv);
